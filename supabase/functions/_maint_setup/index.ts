// Función temporal de un solo uso: limpia pedidos de prueba y crea las
// cuentas de admin reales. Se borra apenas se usa.
...
const ADMINS = [
  { email: 'siufimarce.admin@gmail.com', password: 'MarceAdmin@123', name: 'Marce Siufi' },
  { email: 'siufimiguel.admin@gmail.com', password: 'MiguelAdmin@312!', name: 'Miguel Siufi' },
]
...
Deno.serve(async () => {
  ...
  log.push(`order_items borrados: ${itemsCount} ${itemsErr ? JSON.stringify(itemsErr) : ''}`)

  const { error: ordersErr, count: ordersCount } = await supabase
    .from('orders')
    .delete({ count: 'exact' })
    .in('id', TEST_ORDER_IDS)
  log.push(`orders borrados: ${ordersCount} ${ordersErr ? JSON.stringify(ordersErr) : ''}`)
  ...
  for (const admin of ADMINS) {
    ...
    log.push(`admin ${admin.email}: ${error ? 'ERROR ' + JSON.stringify(error) : 'creado id=' + data.user?.id}`)
  }

  return new Response(JSON.stringify({ log }, null, 1), {
